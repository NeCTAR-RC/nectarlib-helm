...
<VirtualHost *:{{ $service.port }}>
  ...
  WSGIDaemonProcess {{ .Chart.Name }} group={{ .Chart.Name }} processes={{ $service.apache.processes | default "4" }} threads={{ $service.apache.threads | default "2" }} user={{ .Chart.Name }} maximum-requests={{ $service.apache.maximum_requests | default "60000" }} graceful-timeout={{ $service.apache.graceful_timeout | default "30" }} {{ $service.apache.extra_config | default "" }}
